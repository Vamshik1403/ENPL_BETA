...
export default function SitesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await fetch(`https://ristarerp.openwan.in/backend/sites/${editingId}`, {
          method: 'PUT',
          headers: {
          ...
          for (const contact of formContacts) {
            if (contact.contactPerson.trim() && contact.designation.trim() && contact.contactNumber.trim() && contact.emailAddress.trim()) {
              if (contact.id) {
                ...
                await fetch(`https://ristarerp.openwan.in/backend/sites/contacts/${contact.id}`, {
                  method: 'PUT',
                ...
                // Create new contact
                await fetch(`https://ristarerp.openwan.in/backend/sites/${editingId}/contacts`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    contactPerson: contact.contactPerson,
        ...
        if (response.ok) {
          ...
          for (const contact of formContacts) {
            if (contact.contactPerson.trim() && contact.designation.trim() && contact.contactNumber.trim() && contact.emailAddress.trim()) {
              await fetch(`https://ristarerp.openwan.in/backend/sites/${newSite.id}/contacts`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                  contactPerson: contact.contactPerson,
  ...
  const handleEdit = async (id: number) => {
    ...
    if (item) {
      ...
      try {
        const response = await fetch(`https://ristarerp.openwan.in/backend/sites/${id}/contacts`);
        if (response.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (confirm('Are you sure you want to delete this site?')) {
      try {
        setLoading(true);
        const response = await fetch(`https://ristarerp.openwan.in/backend/sites/${id}`, {
          method: 'DELETE',
        });
  ...
  const fetchCustomerFullData = async (id: number) => {
    try {
      const res = await fetch(`https://ristarerp.openwan.in/backend/address-book/${id}`);
      const data = await res.json();
