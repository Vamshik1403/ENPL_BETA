...
export default function CustomerContactPage() {
  const API = 'https://enplerp.electrohelps.in/customer-contact';
  ...
  const fetchSites = async (customerId: number) => {
    ...
    const res = await fetch(
      `https://enplerp.electrohelps.in/sites/based-on-cust?addressBookId=${customerId}`,
    );
...
useEffect(() => {
  ...
  const deleteMappedSite = async (mappingId: number) => {
    ...
    await fetch(
      `https://enplerp.electrohelps.in/customer-contact/site/${mappingId}`,
      { method: 'DELETE' },
    );
    fetchAll();
  };
