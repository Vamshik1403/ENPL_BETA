...
export default function CustomerContactPage() {
  const API = 'https://enplerp.electrohelps.in/backend/api/customer-contact';
  ...
  const fetchSites = async (customerId: number) => {
    ...
    const res = await fetch(
      `https://enplerp.electrohelps.in/backend/api/sites/based-on-cust?addressBookId=${customerId}`,
    );
...
useEffect(() => {
  ...
  const deleteMappedSite = async (mappingId: number) => {
    ...
    await fetch(
      `https://enplerp.electrohelps.in/backend/api/customer-contact/site/${mappingId}`,
      { method: 'DELETE' },
    );
    fetchAll();
  };
