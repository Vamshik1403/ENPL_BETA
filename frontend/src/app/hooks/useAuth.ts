...
export const useAuth = () => {
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    ...
    try {
      // 1️⃣ Login
      const res = await fetch("https://enplerp.electrohelps.in/backend/api/auth/login", {
        method: "POST",
      ...
      const usersRes = await fetch("https://enplerp.electrohelps.in/backend/api/auth/users", {
