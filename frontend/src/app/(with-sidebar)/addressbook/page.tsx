...
export default function AddressBookPage() {
  ...
  const generateAddressBookId = async (addressType: string) => {
    try {
      const response = await fetch(`https://enplerp.electrohelps.in/backend/api/address-book/next-id/${addressType}`);
      const data = await response.json();
      return data.nextId;
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        const response = await fetch(`https://enplerp.electrohelps.in/backend/api/address-book/${editingId}`, {
          method: 'PUT',
          headers: {
            ...
            for (const contact of formContacts) {
              if (contact.contactPerson.trim() && contact.designation.trim() && contact.contactNumber.trim() && contact.emailAddress.trim()) {
                if (contact.id) {
                  ...
                  const contactResponse = await fetch(`https://enplerp.electrohelps.in/backend/api/address-book/contacts/${contact.id}`, {
                    method: 'PUT',
                  ...
                  const contactResponse = await fetch('https://enplerp.electrohelps.in/backend/api/addressbookcontact', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      ...contact,
        ...
        if (response.ok) {
          ...
          for (const contact of formContacts) {
            if (contact.contactPerson.trim() && contact.designation.trim() && contact.contactNumber.trim() && contact.emailAddress.trim()) {
              await fetch('https://enplerp.electrohelps.in/backend/api/addressbookcontact', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
  ...
  const handleEdit = async (id: number) => {
    ...
    if (item) {
      ...
      try {
        const response = await fetch(`https://enplerp.electrohelps.in/backend/api/address-book/${id}/contacts`);
        if (response.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (confirm('Are you sure you want to delete this customer?')) {
      try {
        const response = await fetch(`https://enplerp.electrohelps.in/backend/api/address-book/${id}`, {
          method: 'DELETE',
        });
