...
export default function CustomerComplaintPage() {
  ...
  const fetchTasksByCustomers = async (customerIds: number[]) => {
    setIsLoading(true);
    try {
      const res = await fetch('https://enplerp.electrohelps.in/backend/api/task/by-customers', {
        method: 'POST',
  ...
  const fetchMappedCustomers = async (customerIds: number[]) => {
    try {
      const res = await fetch('https://enplerp.electrohelps.in/backend/api/address-book');
  ...
  const fetchCustomerSites = async (addressBookId: number) => {
    try {
      const res = await fetch(
        `https://enplerp.electrohelps.in/backend/api/sites/based-on-cust?addressBookId=${addressBookId}`,
      );
      const data = await res.json();
  ...
  const handleCompleteTask = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://enplerp.electrohelps.in/backend/api/task/${taskToComplete.id}/customer-remark`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          const response = await fetch(
                            `https://enplerp.electrohelps.in/backend/api/task/${activeTask.id}/customer-remark`,
                            {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify(payload),
